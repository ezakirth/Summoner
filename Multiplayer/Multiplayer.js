...
function Multiplayer:init(dcb, ccb)
{
    ...
    this.peer_ip, this.peer_port = nil, this.my_port;
    ...
    this.dataCallback = dcb or function() }
    this.connectedCallback = ccb or function() }
}
...
{
    var data, msg_or_ip, port_or_nil = this.client:receivefrom();
    if ( data ) {
            // Store the ip of this new client so you can send data back;
            this.peer_ip, this.peer_port = msg_or_ip, port_or_nil;
            
            if ( not this.connected and data == "connection_confirmation" ) {
                this:sendData("connection_confirmation");
